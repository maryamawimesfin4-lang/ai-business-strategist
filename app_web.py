...
# Page Configuration
st.set_page_config(page_title="AI Business Strategist", page_icon="🚀", layout="centered")
...
# Streamlit Cloud will read this key from secrets automatically
...
def marketing_node(state: MiniMarketState) -> dict:
    prompt = f"Create a 3-bullet marketing strategy for this business concept: {state['market_concept']}"
    res = llm.invoke(prompt)
    return {"marketing_plan": clean_text(res)}

def sales_node(state: MiniMarketState) -> dict:
    prompt = f"Based on this marketing strategy:\n{state['marketing_plan']}\n\nList 3 key sales tactics and channels."
    res = llm.invoke(prompt)
    return {"sales_strategy": clean_text(res)}

def operations_node(state: MiniMarketState) -> dict:
    prompt = f"For the business concept '{state['market_concept']}', outline a 3-step production and supply operations plan."
    res = llm.invoke(prompt)
...
if st.button("Generate Complete Blueprint", type="primary"):
    if market_concept.strip():
        ...
        # Allow user to download the final report directly from the web browser
        report_text = f"""BUSINESS BLUEPRINT: {result['market_concept']}
